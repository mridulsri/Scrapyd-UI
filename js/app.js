...
var scrapyd_url = 'http://172.16.2.233:6800/';
var scrapyd_php_url = 'http://localhost:90/scrapyd-webapp/';
...
{   
    //screen reset conditions
    if ($('#scrapydurl').val() === '' || scrapyd_url === undefined)
    {
        scrapyd_url = 'http://172.16.2.233:6800/';
        $('#scrapydurl').val(scrapyd_url)
    }
       
    $('#available_spiders').html('<tr><td colspan="2">Select a project</td></tr>')
    ...
    $("#scrapydurl").on('input propertychange paste', function () {
        if ($('#scrapydurl').val() === '' || scrapyd_url === undefined) {
            scrapyd_url = 'http://172.16.2.233:6800/';
            $('#scrapydurl').val(scrapyd_url)
        }
        else {
            scrapyd_url = $('#scrapydurl').val();
    ...
    $.getJSON( scrapyd_url+"listprojects.json", {} ).done(function( json )
        {
            ...
            {
                var one_project = '';
                // one_project += '<a class="dropdown-item" href="javascript:show_project(\''+ val + '\');">';
                one_project += '<option  value="'+ val +'">'
                    one_project += val;
                one_project += '</option>';
...
function get_file(file)
{
    ...
    $.ajax({
        type: "POST",
        dataType: 'text',
        url: scrapyd_php_url+'process.php',
        data: {
            'file':file
        },
        crossDomain : true,
    ...
    .fail( function(xhr, textStatus, errorThrown)
    {
        ...
        $('#spider_content').css('display', 'block')
        $('#spider_content_iframe').attr('src', file);
